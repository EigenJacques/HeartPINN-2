...
import tfp_lbfgs_utils as bfgs_utils
...
def minimize(value_and_gradients_function,
  ...
  with tf.name_scope(name or 'minimize'):
    ...
    def _body(current_state):
      ...
      # print and log optimizer state
      if current_state.num_iterations%int(max_iterations/10) == 0:
        logger(current_state.num_iterations,current_state.position)
